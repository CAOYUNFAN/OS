...
void * next_cluster(void * ptr,u32 rowsize,int tag,int len){
  ...
  LL res=nxtptr<end_of_file?chk((u8 *)nxtptr-rowsize,nxtptr,len):MAXNN * MAXNN * rowsize;
  if(nxtptr<end_of_file && type[num]!=1 && type[num]!=(u16)(-1) && type[num]!=2 && type[num]!=tag &&( res<MAXNN * rowsize/2 || (type[num]==0 && res < MAXNN *rowsize) )) {
    type[num]=tag;
    return nxtptr;
  }
